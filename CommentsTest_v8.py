    ...
    for thread in video_comment_threads:
        try:
            ...
            for k in sorted(ss):
                ...
                if(ss[k] < 0):
                    total_video_score[k] = total_video_score[k]-(ss[k]**4)
                else:
                    total_video_score[k] = total_video_score[k]+(ss[k]**4)
...
##            print(text)
    ...
    for k in sorted(total_video_score):
        if(total_video_score[k] < 0):
            total_video_score[k] = -1*(abs(total_video_score[k])**(1/4))/len(comments);
        else:
            total_video_score[k] = (abs(total_video_score[k])**(1/4))/len(comments);
