...
if __name__ == "__main__":
  ...
  try:
    comments = []
    video_comment_threads = get_comment_threads(youtube, '7fwbtZUk8vw', comments)
    ...
    for thread in video_comment_threads:
        try:
            ...
            for k in sorted(ss):
             ...
             total_video_score[k] = total_video_score[k]+(ss[k]**2)
            print()
            print()
            print(text)
    ...
    for k in sorted(total_video_score):
        print('{0}: {1}, '.format(k, total_video_score[k]**(0.5)), end='')
    print()
