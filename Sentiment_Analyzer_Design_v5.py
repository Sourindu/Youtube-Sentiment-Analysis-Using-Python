...
##test_set = classifier_training[60000:]
classifier = nltk.NaiveBayesClassifier.train(train_set);
##nltk.classify.accuracy(classifier, test_set);

##testing simple text

text = word_tokenize("This is terrible");
tgd = t2.tag(text);

t,v =[w for w,k in tgd], [k for w,k in tgd];
...
print('input : ',text,'classification : ',classifier.classify(f))

text = word_tokenize("The video is poorly presented");
tgd = t2.tag(text);
t=[w for w,k in tgd];
v=[k for w,k in tgd];
arr=[];
